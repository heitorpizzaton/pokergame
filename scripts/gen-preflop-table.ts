...
type Matchup = { key: number; a: Hand; b: Hand };
type Result = { key: number; values: number[] };
...
function runWorker(): void {
  ...
  for (const m of matchups) {
    const [ra, rb] = exactEquityVsHands(
      [m.a as unknown as Card[], m.b as unknown as Card[]],
      [],
    );
...
async function main(): Promise<void> {
  ...
  const body = rows
    .map(
      (r) =>
        `  ['${r.label}', ${r.combos}, ${fmt(r.win)}, ${fmt(r.tie)}, ${fmt(r.equity)}],`,
    )
    .join('\n');
